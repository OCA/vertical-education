...
class OpStudentFeesDetails(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('invoice', 'Invoice Created'), ('cancel', 'Cancel')], string='Status', copy=False)
    invoice_state = fields.Selection(related='invoice_id.state', string='Invoice Status', readonly=True)
    company_id = fields.Many2one('res.company', default=lambda self: self.env.user.company_id)
    after_discount_amount = fields.Monetary(compute='_compute_discount_amount', currency_field='currency_id')
    ...
    currency_id = fields.Many2one('res.currency', compute='_compute_currency_id', default=lambda self: self.env.user.company_id.currency_id.id)
    ...
    def get_invoice(self):
        ...
        if self.amount <= 0.0:
            ...
            amount = self.amount
            name = product.name
        element_id = self.env['op.fees.element'].search([('fees_terms_line_id', '=', self.fees_line_id.id)])
        invoice_line_list = []
        if element_id:
            for records in element_id:
                invoice_line_list.append((0, 0, {'name': records.product_id.name, 'account_id': account_id, 'price_unit': records.value * self.amount / 100, 'quantity': 1.0, 'discount': self.discount or False, 'product_uom_id': records.product_id.uom_id.id, 'product_id': records.product_id.id}))
        else:
            invoice_line_list.append((0, 0, {'name': name, 'account_id': account_id, 'price_unit': amount, 'quantity': 1.0, 'discount': self.discount or False, 'product_uom_id': product.uom_id.id, 'product_id': product.id}))
        invoice = inv_obj.create({'move_type': 'out_invoice', 'partner_id': partner_id.id, 'invoice_line_ids': invoice_line_list})
    ...
    def action_get_invoice(self):
        ...
        if self.invoice_id:
            ...
            value = {'domain': str([('id', '=', self.invoice_id.id)]), 'view_type': 'form', 'view_mode': 'form', 'res_model': 'account.move', 'view_id': False, 'views': [(form_view and form_view.id or False, 'form'), (tree_view and tree_view.id or False, 'tree')], 'type': 'ir.actions.act_window', 'res_id': self.invoice_id.id, 'target': 'current', 'nodestroy': True}
        return value

class OpStudent(models.Model):
    _inherit = 'op.student'
    fees_detail_ids = fields.One2many('op.student.fees.details', 'student_id', string='Fees Collection Details', tracking=True)
    fees_details_count = fields.Integer(compute='_compute_fees_details')

    @api.depends('fees_detail_ids')
    def _compute_fees_details(self):
        for fees in self:
            fees.fees_details_count = self.env['op.student.fees.details'].search_count([('student_id', '=', self.id)])
    ...
    def action_view_invoice(self):
        ...
        for student in self:
            ...
            result['context'] = {'default_partner_id': student.partner_id.id, 'default_move_type': 'out_invoice'}
        if len(inv_ids) > 1:
