...
class TestMedia(TestLibraryCommon):
    ...
    def test_case_media(self):
        ...
        if not media:
            raise AssertionError('Error in data, please check for library media details')
...
class TestMediaUnit(TestLibraryCommon):
    ...
    def test_case_media_unit(self):
        mediaunit = self.op_media_unit.create({'name': 'Troposhere In Detail unit-006', 'barcode': 'MUB0000067', 'media_id': self.env.ref('openeducat_library.op_media_1').id})
...
class TestMediaMovement(TestLibraryCommon):
    ...
    def test_case_media_movement(self):
        ...
        if not mediamovement:
            raise AssertionError('Error in data, please check for library media movement details')
...
class TestMediaPurchase(TestLibraryCommon):
    ...
    def test_case_media_purchase(self):
        mediapurchase = self.op_media_purchase.create({'name': 'Accounting Made Simple', 'author': 'Mike Piper', 'requested_id': self.env.ref('openeducat_core.op_res_partner_1').id, 'course_ids': self.env.ref('openeducat_core.op_course_1').id, 'subject_ids': self.env.ref('openeducat_core.op_subject_1').id})
...
class TestMediaQueue(TestLibraryCommon):
    ...
    def test_case_media_queue(self):
        mediaqueue = self.op_media_queue.create({'name': 'QUE001', 'media_id': self.env.ref('openeducat_library.op_media_1').id, 'date_from': time.strftime('%Y-%m-01'), 'date_to': time.strftime('%Y-%m-01')})
...
class TestLibraryCardType(TestLibraryCommon):
    ...
    def test_case_library_card_type(self):
        ...
        if not cardtype:
            raise AssertionError('Error in data, please check for library card type details')
...
class TestLibraryCard(TestLibraryCommon):
    ...
    def test_case_library_card(self):
        card = self.op_library_card.create({'partner_id': self.env.ref('openeducat_core.op_res_partner_1').id, 'number': 'LCB0000000018', 'library_card_type_id': self.env.ref('openeducat_library.op_library_card_type_1').id, 'issue_date': time.strftime('%Y-%m-01'), 'type': 'student'})
        if not card:
...
class TestWizardIssue(TestLibraryCommon):
    ...
    def test_case_wizard_issue(self):
        wizard = self.wizard_issue.create({'media_id': self.env.ref('openeducat_library.op_media_1').id, 'media_unit_id': self.env.ref('openeducat_library.op_media_unit_3').id, 'type': 'student', 'library_card_id': self.env.ref('openeducat_library.op_library_card_type_1').id, 'issued_date': time.strftime('%Y-%m-01'), 'return_date': time.strftime('%Y-%m-01')})
        wizard.do_issue()
        wizard._check_date()
...
class TestWizardReserve(TestLibraryCommon):
    ...
    def test_case_wizard_reserve(self):
        reserve = self.reserve_media.create({'partner_id': self.env.ref('openeducat_core.op_res_partner_1').id})
        reserve.set_partner()
...
class TestWizardReturn(TestLibraryCommon):
    ...
    def test_case_wizard_return(self):
        return_wizard = self.return_media.create({'media_id': self.env.ref('openeducat_library.op_media_1').id, 'media_unit_id': self.env.ref('openeducat_library.op_media_unit_2').id})
