...
class IssueMedia(models.TransientModel):
    ...
    def check_max_issue(self, student_id, library_card_id):
        ...
        if (
            len(media_movement_search)
            < self.env["op.library.card"]
            .browse(library_card_id)
            .library_card_type_id.allow_media
        ):
            return True
        else:
            return False
        ...
        for media in self:
            ...
            if media.check_max_issue(media.student_id.id, media.library_card_id.id):
                if (
                    ...
                    and media.media_unit_id.state == "available"
                ):
                    ...
                    if media.type == "student":
                        media_movement_create["partner_id"] = (
                            media.student_id.partner_id.id or False
                        )
                    elif media.type == "faculty":
                        media_movement_create["partner_id"] = (
                            media.faculty_id.partner_id.id or False
                        )
                    else:
                    ...
                    raise UserError(
                        _("Media unit can not be issued because it's already: %s")
                        % (
                            dict(  # noqa
                                media.media_unit_id._fields["state"].selection
                            ).get(media.media_unit_id.state)
                        )
                    )
            else:
                raise UserError(
