...
class BonafideCertificateWizard(models.TransientModel):
    ...
    certificate_purpose = fields.Selection([('passport', 'Passport Application'), ('bank_account', 'Bank Account Opening'), ('scholarship', 'Scholarship'), ('internship', 'Internship'), ('visa', 'Visa'), ('address_proof', 'Address Proof'), ('other', 'Other')], required=True, default='passport')
    ...
    def action_print_bonafide_certificate(self):
        ...
        action = self.env.ref('openeducat_core.action_report_student_bonafide').report_action(students.ids, data={'certificate_purpose': self.certificate_purpose, 'other_purpose': self.other_purpose, 'purpose_display': self._get_purpose_display()})
        action['close_on_report_download'] = True
        return action

    def _get_purpose_display(self):
        """Get the display text for the selected purpose."""
        purpose_mapping = {'passport': 'Passport Application', 'bank_account': 'Bank Account Opening', 'scholarship': 'Scholarship', 'internship': 'Internship', 'visa': 'Visa', 'address_proof': 'Address Proof', 'other': self.other_purpose or 'Other'}
