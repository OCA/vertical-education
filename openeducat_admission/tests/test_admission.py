...
class TestAdmission(TestAdmissionCommon):
    ...
    def test_case_1_admissions(self):
        ...
        vals = {'name': 'AHIR', 'first_name': 'Nikul', 'middle_name': 'M', 'last_name': 'Last', 'application_number': self.env.ref('openeducat_admission.seq_op_admission').id, 'birth_date': '2002-12-20', 'course_id': self.env.ref('openeducat_core.op_course_5').id, 'batch_id': self.env.ref('openeducat_core.op_batch_4').id, 'phone': 32234234, 'mobile': 93432, 'email': 'nik.ahir@gmail.com', 'state': 'submit', 'gender': 'm', 'register_id': self.env.ref('openeducat_admission.op_admission_register_3').id, 'image': False}
        studnet_1 = self.op_admission.create(vals)
        studnet_1.enroll_student()
        vals_2 = {'name': 'Ahir', 'first_name': 'Nikul', 'middle_name': 'M', 'last_name': 'Last', 'birth_date': '2002-12-20', 'course_id': self.env.ref('openeducat_core.op_course_5').id, 'batch_id': self.env.ref('openeducat_core.op_batch_4').id, 'phone': 32234234, 'mobile': 93432, 'email': 'nisak.ahir@gmail.com', 'state': 'submit', 'gender': 'm', 'register_id': self.env.ref('openeducat_admission.op_admission_register_3').id, 'student_id': self.env.ref('openeducat_core.op_student_18').id, 'fees_term_id': 2, 'fees': 1000}
...
class TestAdmissionAnalysisWizard(TestAdmissionCommon):
    ...
    def test_wizard_admission_analysis(self):
        vals = {'course_id': self.env.ref('openeducat_core.op_course_2').id, 'start_date': '2018-01-01', 'end_date': '2019-12-30'}
