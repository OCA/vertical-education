            ...
            if data.register_id:
                if data.register_id.admission_base == "program":
                    ...
                    for rec in data.register_id.admission_fees_line_ids:
                        course_list.append(
                            rec.course_id.id
                        ) if rec.course_id.id not in course_list else None  # noqa
    ...
    def create_sequence(self):
        if not self.application_number:
            self.application_number = (
                self.env["ir.sequence"].next_by_code("op.admission") or "/"
            )
    ...
    def get_student_vals(self):
        ...
        for student in self:
                ...
                student_user = self.env["res.users"].create(
                    {
                        ...
                        "login": student.email
                        if student.email
                        else student.application_number,  # noqa
            ...
            details = {
                ...
                "city": student.city,
                "country_id": student.country_id and student.country_id.id or False,
            ...
            if not record.student_id:
                ...
                if vals:
                    record.student_id = student_id = (
                        self.env["op.student"].create(vals).id
                    )
                    record.partner_id = (
                        record.student_id.partner_id.id if record else False
                    )
                        ...
                        "course_detail_ids": [
                            [
                                ...
                                False,
                                {
                                    ...
                                    and record.course_id.id
                                    or False,
                                    "batch_id": record.batch_id
                                    and record.batch_id.id
                                    or False,
            ...
            if record.fees_term_id.fees_terms in ["fixed_days", "fixed_date"]:
                ...
                for line in record.fees_term_id.line_ids:
                    ...
                    if line.due_date:
                        ...
                        date_now = (
                            datetime.today() + relativedelta(days=no_days)
                        ).date()
                        dict_val.update(
                            {
                                "date": date_now,
