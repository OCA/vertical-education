...
class OpAdmissionRegister(models.Model):
    ...
    def _compute_application_counts(self):
        for record in self:
            record.draft_count = (
                record.admission_ids.filtered(lambda a: a.state == "draft")
                .mapped("id")
                .__len__()
            )
            record.online_count = (
                record.admission_ids.filtered(lambda a: a.state == "online")
                .mapped("id")
                .__len__()
            )
        ...
        for record in self:
            ...
            if record.min_count > record.max_count:
                raise ValidationError(
                    _("Min Admission can't be greater than Max Admission")
                )
