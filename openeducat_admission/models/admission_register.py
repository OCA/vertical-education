...
class OpAdmissionRegister(models.Model):
    ...
    end_date = fields.Date(required=False, readonly=True, default=fields.Date.today() + relativedelta(days=30))
    ...
    state = fields.Selection([('draft', 'Draft'), ('confirm', 'Confirmed'), ('cancel', 'Cancelled'), ('application', 'Application Gathering'), ('admission', 'Admission Process'), ('done', 'Done')], 'Status', default='draft', tracking=True)
    ...
    def _compute_application_counts(self):
        for record in self:
            record.draft_count = record.admission_ids.filtered(lambda a: a.state == 'draft').mapped('id').__len__()
            record.online_count = record.admission_ids.filtered(lambda a: a.state == 'online').mapped('id').__len__()
    ...
    def check_no_of_admission(self):
        for record in self:
            ...
            if record.min_count > record.max_count:
                raise ValidationError(_("Min Admission can't be greater than Max Admission"))

    def open_student_application(self):
        return {'type': 'ir.actions.act_window', 'res_model': 'op.admission', 'domain': [('register_id', '=', self.id)], 'name': 'Applications', 'view_mode': 'list,form'}
    ...
    def action_open_draft_courses(self):
        return {'name': 'Draft Admissions', 'type': 'ir.actions.act_window', 'view_mode': 'list,form', 'res_model': 'op.admission', 'domain': [('id', 'in', self.admission_ids.ids), ('state', '=', 'draft')], 'target': 'current'}

    def action_open_confirmed_courses(self):
        return {'name': 'Confirmed Courses', 'type': 'ir.actions.act_window', 'view_mode': 'list,form', 'res_model': 'op.admission', 'domain': [('id', 'in', self.admission_ids.ids), ('state', '=', 'confirm')], 'target': 'current'}

    def action_open_enrolled_courses(self):
        return {'name': 'Enrolled Courses', 'type': 'ir.actions.act_window', 'view_mode': 'list,form', 'res_model': 'op.admission', 'domain': [('id', 'in', self.admission_ids.ids), ('state', '=', 'done')], 'target': 'current'}

    def action_open_online_courses(self):
        return {'name': 'Enrolled Courses', 'type': 'ir.actions.act_window', 'view_mode': 'list,form', 'res_model': 'op.admission', 'domain': [('id', 'in', self.admission_ids.ids), ('state', '=', 'online')], 'target': 'current'}
