...
class StudentMigrate(models.TransientModel):
    ...
    def _check_admission_register(self):
        for record in self:
            ...
            if (record.course_from_id.parent_id and record.course_to_id) or (
                record.course_from_id.parent_id and record.course_completed
            ):
                    ...
                    if record.course_from_id.parent_id != record.course_to_id.parent_id:
                        raise ValidationError(
                            _(
                                ...
                                "don't share same parent course!"
                            )
                        )
            elif (record.course_from_id.program_id and record.course_to_id) or (
                record.course_from_id.program_id and record.course_completed
            ):
                if record.course_to_id:
                    if (
                        record.course_from_id.program_id
                            ...
                            reg_id = self.env["op.subject.registration"].create(
                                {
                                    ...
                                    "batch_id": record.batch_id.id,
                                    "course_id": record.course_to_id.id,
                                    "min_unit_load": record.course_to_id.min_unit_load
                                    or 0.0,
                                    "max_unit_load": record.course_to_id.max_unit_load
                                    or 0.0,
                                    "state": "draft",
                                }
                            )
