...
class GenerateSession(models.TransientModel):
    ...
    time_table_lines_1 = fields.One2many('gen.time.table.line', 'gen_time_table', 'Time Table Lines1', domain=[('day', '=', '0')])
    time_table_lines_2 = fields.One2many('gen.time.table.line', 'gen_time_table', 'Time Table Lines2', domain=[('day', '=', '1')])
    time_table_lines_3 = fields.One2many('gen.time.table.line', 'gen_time_table', 'Time Table Lines3', domain=[('day', '=', '2')])
    time_table_lines_4 = fields.One2many('gen.time.table.line', 'gen_time_table', 'Time Table Lines4', domain=[('day', '=', '3')])
    time_table_lines_5 = fields.One2many('gen.time.table.line', 'gen_time_table', 'Time Table Lines5', domain=[('day', '=', '4')])
    time_table_lines_6 = fields.One2many('gen.time.table.line', 'gen_time_table', 'Time Table Lines6', domain=[('day', '=', '5')])
    time_table_lines_7 = fields.One2many('gen.time.table.line', 'gen_time_table', domain=[('day', '=', '6')])
    start_date = fields.Date(required=True, default=time.strftime('%Y-%m-01'))
    ...
    def change_tz(self, date):
        ...
        return datetime.datetime.strptime(utc_dt, '%Y-%m-%d %H:%M:%S')
            ...
            for n in range((end_date - start_date).days + 1):
                ...
                for line in session.time_table_lines:
                    if int(line.day) == curr_date.weekday():
                        ...
                        final_start_date = datetime.datetime.strptime(curr_date.strftime('%Y-%m-%d ') + session_start_time, '%Y-%m-%d %H:%M:%S')
                        final_end_date = datetime.datetime.strptime(curr_date.strftime('%Y-%m-%d ') + session_end_time, '%Y-%m-%d %H:%M:%S')
                        curr_start_date = self.change_tz(final_start_date)
                        curr_end_date = self.change_tz(final_end_date)
                        data.append({'faculty_id': line.faculty_id.id, 'subject_id': line.subject_id.id, 'course_id': session.course_id.id, 'batch_id': session.batch_id.id, 'classroom_id': line.classroom_id.id, 'start_datetime': curr_start_date.strftime('%Y-%m-%d %H:%M:%S'), 'end_datetime': curr_end_date.strftime('%Y-%m-%d %H:%M:%S'), 'type': calendar.day_name[int(line.day)]})
            if data:
...
class GenerateSessionLine(models.TransientModel):
    ...
    day = fields.Selection([('0', calendar.day_name[0]), ('1', calendar.day_name[1]), ('2', calendar.day_name[2]), ('3', calendar.day_name[3]), ('4', calendar.day_name[4]), ('5', calendar.day_name[5]), ('6', calendar.day_name[6])], required=True)