...
week_days = [
    (calendar.day_name[0], (calendar.day_name[0])),
...
class OpSession(models.Model):
    ...
    def _compute_timing(self):
        ...
        for session in self:
            session.timing = (
                str(session.start_datetime.astimezone(tz).strftime("%I:%M%p"))
                + " - "
            ...
            for rec in self:
                if rec.id != session.id:
                    if is_faculty_constraint:
                        if (
                            rec.faculty_id.id == session.faculty_id.id
                            and rec.start_datetime.date()
                            == session.start_datetime.date()
                            and (  # noqa
                                session.start_datetime.time()
                                < rec.start_datetime.time()
                                < session.end_datetime.time()  # noqa
                                or session.start_datetime.time()
                                < rec.end_datetime.time()
                                < session.end_datetime.time()  # noqa
                                or rec.start_datetime.time()
                                <= session.start_datetime.time()
                                < rec.end_datetime.time()  # noqa
                                or rec.start_datetime.time()
                                < session.end_datetime.time()
                                <= rec.end_datetime.time()
                            )
                        ):  # noqa
                    ...
                    if is_classroom_constraint:
                        if (
                            rec.classroom_id.id == session.classroom_id.id
                            and rec.start_datetime.date()
                            == session.start_datetime.date()
                            and (  # noqa
                                session.start_datetime.time()
                    ...
                    if is_batch_and_subject_constraint:
                        if (
                            rec.batch_id.id == session.batch_id.id
                            and rec.start_datetime.date()
                            == session.start_datetime.date()
                            and (  # noqa
                                session.start_datetime.time()
    ...
    def notify_user(self):
        for session in self:
            template = self.env.ref(
                "openeducat_timetable.session_details_changes", raise_if_not_found=False
            )
    ...
    def write(self, vals):
        data = super(OpSession, self.with_context(check_move_validity=False)).write(
            vals
        )
        for session in self:
