...
week_days = [(calendar.day_name[0], calendar.day_name[0]), (calendar.day_name[1], calendar.day_name[1]), (calendar.day_name[2], calendar.day_name[2]), (calendar.day_name[3], calendar.day_name[3]), (calendar.day_name[4], calendar.day_name[4]), (calendar.day_name[5], calendar.day_name[5]), (calendar.day_name[6], calendar.day_name[6])]
...
class OpSession(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('confirm', 'Confirmed'), ('done', 'Done'), ('cancel', 'Canceled')], string='Status', default='draft')
    user_ids = fields.Many2many('res.users', compute='_compute_batch_users', store=True, string='Users')
    active = fields.Boolean(default=True)
    company_id = fields.Many2one('res.company', default=lambda self: self.env.user.company_id)
    days = fields.Selection([('monday', 'Monday'), ('tuesday', 'Tuesday'), ('wednesday', 'Wednesday'), ('thursday', 'Thursday'), ('friday', 'Friday'), ('saturday', 'Saturday'), ('sunday', 'Sunday')], group_expand='_expand_groups', store=True)
    ...
    def _compute_name(self):
        tz = pytz.timezone(self.env.user.tz)
        for session in self:
            if session.faculty_id and session.subject_id and session.start_datetime and session.end_datetime:
                session.name = session.faculty_id.name + ':' + session.subject_id.name + ':' + str(session.start_datetime.astimezone(tz).strftime('%I:%M%p')) + '-' + str(session.end_datetime.astimezone(tz).strftime('%I:%M%p'))
    ...
    def _compute_batch_users(self):
        ...
        for session in self:
            student_ids = student_obj.search([('course_detail_ids.batch_id', '=', session.batch_id.id)])
            user_list = [student_id.user_id.id for student_id in student_ids if student_id.user_id]
    ...
    @api.constrains('faculty_id', 'start_datetime', 'end_datetime', 'classroom_id', 'batch_id', 'subject_id')
            ...
            for rec in self:
                if rec.id != session.id:
                    if is_faculty_constraint:
                        if rec.faculty_id.id == session.faculty_id.id and rec.start_datetime.date() == session.start_datetime.date() and (session.start_datetime.time() < rec.start_datetime.time() < session.end_datetime.time() or session.start_datetime.time() < rec.end_datetime.time() < session.end_datetime.time() or rec.start_datetime.time() <= session.start_datetime.time() < rec.end_datetime.time() or (rec.start_datetime.time() < session.end_datetime.time() <= rec.end_datetime.time())):
                            raise ValidationError(_('You cannot create a session with same faculty on same date and time'))
                    if is_classroom_constraint:
                        if rec.classroom_id.id == session.classroom_id.id and rec.start_datetime.date() == session.start_datetime.date() and (session.start_datetime.time() < rec.start_datetime.time() < session.end_datetime.time() or session.start_datetime.time() < rec.end_datetime.time() < session.end_datetime.time() or rec.start_datetime.time() <= session.start_datetime.time() < rec.end_datetime.time() or (rec.start_datetime.time() < session.end_datetime.time() <= rec.end_datetime.time())):
                            raise ValidationError(_('You cannot create a session with same classroom on same date and time'))
                    if is_batch_and_subject_constraint:
                        if rec.batch_id.id == session.batch_id.id and rec.start_datetime.date() == session.start_datetime.date() and (session.start_datetime.time() < rec.start_datetime.time() < session.end_datetime.time() or session.start_datetime.time() < rec.end_datetime.time() < session.end_datetime.time() or rec.start_datetime.time() <= session.start_datetime.time() < rec.end_datetime.time() or (rec.start_datetime.time() < session.end_datetime.time() <= rec.end_datetime.time())) and (rec.subject_id.id == session.subject_id.id):
                            raise ValidationError(_('You cannot create a session for the same batch on same time and for same subject'))
                    if is_batch_constraint:
                        if rec.batch_id.id == session.batch_id.id and rec.start_datetime.date() == session.start_datetime.date() and (session.start_datetime.time() < rec.start_datetime.time() < session.end_datetime.time() or session.start_datetime.time() < rec.end_datetime.time() < session.end_datetime.time() or rec.start_datetime.time() <= session.start_datetime.time() < rec.end_datetime.time() or (rec.start_datetime.time() < session.end_datetime.time() <= rec.end_datetime.time())):
                            raise ValidationError(_('You cannot create a session for the same batch on same time even if it is different subject'))
    ...
    def create(self, values):
        ...
        for record in records:
            ...
            if record.batch_id and record.course_id:
                course_val = self.env['op.student.course'].search([('batch_id', '=', record.batch_id.id), ('course_id', '=', record.course_id.id)])
            ...
            if partner_ids and subtype_id:
                ...
                for partner in list(set(partner_ids)):
                    if partner in partner_val:
                        continue
                    mail_followers.create({'res_model': record._name, 'res_id': record.id, 'partner_id': partner, 'subtype_ids': [[6, 0, [subtype_id[0].id]]]})
        return records
    ...
    def notify_user(self):
        for session in self:
            template = self.env.ref('openeducat_timetable.session_details_changes', raise_if_not_found=False)
    ...
    def get_subject(self):
        return 'Lecture of ' + self.faculty_id.name + ' for ' + self.subject_id.name + ' is ' + self.state

    def write(self, vals):
        data = super(OpSession, self.with_context(check_move_validity=False)).write(vals)
        for session in self:
    ...
    def get_import_templates(self):
        return [{'label': _('Import Template for Sessions'), 'template': '/openeducat_timetable/static/xls/op_session.xls'}]