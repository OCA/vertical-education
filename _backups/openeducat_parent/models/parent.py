...
class OpParent(models.Model):
    ...
    _sql_constraints = [('unique_parent', 'unique(name)', 'Can not create parent multiple times.!')]
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            partner = self.env['res.partner'].search(['|', '|', ('name', '=', parent_name), ('email', '=', parent_email), ('mobile', '=', parent_mobile)], limit=1)
            if partner:
                update_vals = {'is_parent': True}
                if parent_email and (not partner.email):
                    update_vals['email'] = parent_email
                ...
                new_partner = self.env['res.partner'].create({'name': parent_name, 'email': parent_email, 'mobile': parent_mobile, 'is_parent': True})
    ...
    def create_parent_user(self):
        ...
        for record in self:
            ...
            if not record.name.user_id:
                ...
                user_ids = [parent.user_id.id for parent in record.student_ids if parent.user_id]
                user_id = users_res.create({'name': record.name.name, 'partner_id': record.name.id, 'login': record.name.email, 'is_parent': True, 'tz': self._context.get('tz'), 'groups_id': groups_id, 'child_ids': [(6, 0, user_ids)]})
                record.user_id = user_id
                record.name.user_id = user_id

    @api.model
    def get_import_templates(self):
        return [{'label': _('Import Template for Parent'), 'template': '/openeducat_parent/static/xls/op_parent.xls'}]
            ...
            if values.get('parent_ids', False):
                for parent_id in res.parent_ids:
                    if parent_id.user_id:
                        user_ids = [student.user_id.id for student in parent_id.student_ids if student.user_id]
        ...
        if vals.get('parent_ids', False):
            ...
            if self.parent_ids:
                for parent in self.parent_ids:
                    if parent.user_id:
                        user_ids = [parent.user_id.id for parent in parent.student_ids if parent.user_id]
                        parent.user_id.child_ids = [(6, 0, user_ids)]
            else:
                user_ids = self.env['res.users'].search([('child_ids', 'in', self.user_id.id)])
