...
class StudentMigrate(models.TransientModel):
    ...
    def _compute_student_domain(self):
        for record in self:
            if record.course_from_id:
                students = self.env['op.student.course'].search([('course_id', '=', record.course_from_id.id), ('state', '=', 'running')]).mapped('student_id')
    ...
    def _check_admission_register(self):
        for record in self:
            ...
            if record.course_from_id.parent_id and record.course_to_id or (record.course_from_id.parent_id and record.course_completed):
            ...
            elif record.course_from_id.program_id and record.course_to_id or (record.course_from_id.program_id and record.course_completed):
                if record.course_to_id:
                    if record.course_from_id.program_id != record.course_to_id.program_id:
                        raise ValidationError(_("Can't migrate, As selected courses don't share same Program!"))
            else:
                ...
                if self.course_completed:
                    for course_update in student.course_detail_ids:
                        if course_update.course_id == record.course_from_id:
                            ...
                            activity_vals = {'student_id': student.id, 'type_id': act_type.id, 'date': self.date, 'description': _(f'Migration From {record.course_from_id.name} to Completed Course')}
                            self.env['op.activity'].create(activity_vals)
                else:
                    for course_update in student.course_detail_ids:
                        if course_update.course_id == self.course_from_id:
                            course_update.state = 'finished'
                            activity_vals = {'student_id': student.id, 'type_id': act_type.id, 'date': self.date, 'description': _(f'Migration from {record.course_from_id.name} to {record.course_to_id.name}')}
                            self.env['op.activity'].create(activity_vals)
                            student_course = self.env['op.student.course'].search([('student_id', '=', student.id), ('course_id', '=', record.course_from_id.id)])
                            student_course.create({'student_id': student.id, 'course_id': record.course_to_id.id, 'batch_id': record.batch_id.id, 'subject_ids': record.course_to_id.subject_ids.ids})
                            reg_id = self.env['op.subject.registration'].create({'student_id': student.id, 'batch_id': record.batch_id.id, 'course_id': record.course_to_id.id, 'min_unit_load': record.course_to_id.min_unit_load or 0.0, 'max_unit_load': record.course_to_id.max_unit_load or 0.0, 'state': 'draft'})
                            reg_id.get_subjects()
    ...
    def _compute_valid_to_courses(self):
        for rec in self:
            ...
            if rec.course_from_id:
                domain = ['&', '&', ('parent_id', '=', rec.course_from_id.parent_id.id), ('program_id', '=', rec.course_from_id.program_id.id), ('id', '!=', rec.course_from_id.id)]
