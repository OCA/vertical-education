...
class OpAdmission(models.Model):
    ...
    application_date = fields.Datetime(required=True, copy=False, default=lambda self: fields.Datetime.now())
    ...
    fees = fields.Float()
    image = fields.Image()
    state = fields.Selection([('draft', 'Draft'), ('submit', 'Submitted'), ('confirm', 'Confirmed'), ('admission', 'Admission Confirm'), ('reject', 'Rejected'), ('pending', 'Pending'), ('cancel', 'Cancelled'), ('done', 'Done')], default='draft', tracking=True)
    ...
    _sql_constraints = [('unique_application_number', 'unique(application_number)', 'Application Number must be unique per Application!')]
    ...
    def _check_admission_register(self):
        for rec in self:
            ...
            if application_date < start_date or application_date > end_date:
                raise ValidationError(_('Application Date should be between Start Date & End Date of Admission Register.'))
        ...
        for record in self:
            if record.birth_date and record.birth_date > fields.Date.today():
                ...
                if years < self.register_id.minimum_age_criteria:
                    raise ValidationError(_('Not Eligible for Admission minimum required age is : %s') % self.register_id.minimum_age_criteria)

    @api.constrains('name')
    def create_sequence(self):
        if not self.application_number:
            self.application_number = self.env['ir.sequence'].next_by_code('op.admission') or '/'
    ...
    def get_student_vals(self):
        ...
        for student in self:
            ...
            if enable_create_student_user:
                student_user = self.env['res.users'].create({'name': student.name, 'login': student.email if student.email else student.application_number, 'image_1920': self.image or False, 'is_student': True, 'company_id': self.company_id.id, 'groups_id': [(6, 0, [self.env.ref('base.group_portal').id])]})
            details = {'name': student.name, 'phone': student.phone, 'mobile': student.mobile, 'email': student.email, 'street': student.street, 'street2': student.street2, 'city': student.city, 'country_id': student.country_id and student.country_id.id or False, 'state_id': student.state_id and student.state_id.id or False, 'image_1920': student.image, 'zip': student.zip}
            if enable_create_student_user:
                student_user.partner_id.write(details)
            details.update({'title': student.title and student.title.id or False, 'first_name': student.first_name, 'middle_name': student.middle_name, 'last_name': student.last_name, 'birth_date': student.birth_date, 'gender': student.gender if student.gender else False, 'image_1920': student.image or False, 'course_detail_ids': [[0, False, {'course_id': student.course_id and student.course_id.id or False, 'batch_id': student.batch_id and student.batch_id.id or False, 'academic_years_id': student.register_id.academic_years_id.id, 'academic_term_id': student.register_id.academic_term_id.id, 'fees_term_id': student.fees_term_id.id, 'fees_start_date': student.fees_start_date, 'product_id': student.register_id.product_id.id}]], 'user_id': student_user.id if student_user else False, 'company_id': self.company_id.id})
            return details

    def enroll_student(self):
        for record in self:
            if record.register_id.max_count:
                total_admission = self.env['op.admission'].search_count([('register_id', '=', record.register_id.id), ('state', '=', 'done')])
            ...
            if not record.student_id:
                ...
                record.student_id.write({'course_detail_ids': [[0, False, {'course_id': record.course_id and record.course_id.id or False, 'batch_id': record.batch_id and record.batch_id.id or False, 'fees_term_id': record.fees_term_id.id, 'fees_start_date': record.fees_start_date, 'product_id': record.register_id.product_id.id}]]})
            if record.fees_term_id.fees_terms in ['fixed_days', 'fixed_date']:
                ...
                for line in record.fees_term_id.line_ids:
                    ...
                    dict_val = {'fees_line_id': line.id, 'amount': amount, 'fees_factor': per_amount, 'product_id': product_id, 'discount': record.discount or record.fees_term_id.discount, 'state': 'draft', 'course_id': record.course_id and record.course_id.id or False, 'batch_id': record.batch_id and record.batch_id.id or False}
                    if line.due_date:
                        date = line.due_date
                        dict_val.update({'date': date})
                        ...
                        dict_val.update({'date': date_now})
                    val.append([0, False, dict_val])
                record.student_id.write({'fees_detail_ids': val})
            record.write({'nbr': 1, 'state': 'done', 'admission_date': fields.Date.today(), 'student_id': student_id, 'is_student': True})
            reg_id = self.env['op.subject.registration'].create({'student_id': student_id, 'batch_id': record.batch_id.id, 'course_id': record.course_id.id, 'min_unit_load': record.course_id.min_unit_load or 0.0, 'max_unit_load': record.course_id.max_unit_load or 0.0, 'state': 'draft'})
            reg_id.get_subjects()
    ...
    def open_student(self):
        ...
        value = {'domain': str([('id', '=', self.student_id.id)]), 'view_type': 'form', 'view_mode': 'list, form', 'res_model': 'op.student', 'view_id': False, 'views': [(form_view and form_view.id or False, 'form'), (tree_view and tree_view.id or False, 'list')], 'type': 'ir.actions.act_window', 'res_id': self.student_id.id, 'target': 'current', 'nodestroy': True}
        self.state = 'done'
        return value
    ...
    def create_invoice(self):
        ...
        if not account_id:
            raise UserError(_(f'There is no income account defined for this product: "{product.name}". You may have to install a chart of account from Accounting app, settings menu.'))
        if self.fees <= 0.0:
            raise UserError(_('The value of the deposit amount must be positive.'))
        amount = self.fees
        name = product.name
        invoice = self.env['account.invoice'].create({'name': self.name, 'origin': self.application_number, 'move_type': 'out_invoice', 'reference': False, 'account_id': partner_id.property_account_receivable_id.id, 'partner_id': partner_id.id, 'invoice_line_ids': [(0, 0, {'name': name, 'origin': self.application_number, 'account_id': account_id, 'price_unit': amount, 'quantity': 1.0, 'discount': 0.0, 'uom_id': self.register_id.product_id.uom_id.id, 'product_id': product.id})]})
        invoice.compute_taxes()
        form_view = self.env.ref('account.invoice_form')
        tree_view = self.env.ref('account.invoice_tree')
        value = {'domain': str([('id', '=', invoice.id)]), 'view_type': 'form', 'view_mode': 'form', 'res_model': 'account.invoice', 'view_id': False, 'views': [(form_view and form_view.id or False, 'form'), (tree_view and tree_view.id or False, 'list')], 'type': 'ir.actions.act_window', 'res_id': invoice.id, 'target': 'current', 'nodestroy': True}
    ...
    def get_import_templates(self):
        return [{'label': _('Import Template for Admission'), 'template': '/openeducat_admission/static/xls/op_admission.xls'}]

class OpStudentCourseInherit(models.Model):
    _inherit = 'op.student.course'
    product_id = fields.Many2one('product.product', 'Course Fees', domain=[('type', '=', 'service')], tracking=True)

class ResConfigSettings(models.TransientModel):
    _inherit = 'res.config.settings'
    enable_create_student_user = fields.Boolean(config_parameter='openeducat_admission.enable_create_student_user', string='Create Student User')