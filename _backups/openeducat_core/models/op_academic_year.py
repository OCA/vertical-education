...
class OpAcademicYear(models.Model):
    ...
    term_structure = fields.Selection([('two_sem', 'Two Semesters'), ('two_sem_qua', 'Two Semesters Subdivision'), ('two_sem_final', 'Two Semesters Subdivision Final'), ('three_sem', 'Three Trimesters'), ('four_Quarter', 'Four Quarters'), ('final_year', 'Final Year Grades Subdivision'), ('others', 'Other (overlapping terms, custom schedules)')], default='two_sem', required=True)
    ...
    def _handle_two_sem(self):
        ...
        res = [{'name': 'Semester 1', 'from_d': from_d, 'to_d': from_d + timedelta(days=day)}, {'name': 'Semester 2', 'from_d': from_d + timedelta(days=day + 1), 'to_d': to_d}]
        for term in res:
            self.env['op.academic.term'].create({'name': term['name'], 'term_start_date': term['from_d'], 'term_end_date': term['to_d'], 'academic_year_id': self.id})
    ...
    def _handle_two_sem_qua(self):
        ...
        num = 0
        for term in res:
            parent = self.env['op.academic.term'].create({'name': term['name'], 'term_start_date': term['from_d'], 'term_end_date': term['to_d'], 'academic_year_id': self.id})
            for sub_term in self.academic_term_ids:
                ...
                quarters = [{'name': f'Quarter {num + 1}', 'f': s_from, 't': s_from + timedelta(days=s_day)}, {'name': f'Quarter {num + 2}', 'f': s_from + timedelta(days=s_day + 1), 't': s_to}]
                for q in quarters:
                    self.env['op.academic.term'].create({'name': q['name'], 'term_start_date': q['f'], 'term_end_date': q['t'], 'academic_year_id': self.id, 'parent_term': sub_term.id})
                num += 2
    ...
    def _handle_two_sem_final(self):
        ...
        for sub_term in self.academic_term_ids:
            (s_from, s_to) = (sub_term.term_start_date, sub_term.term_end_date)
            s_day = ((s_to - s_from).days + 1) / 2
            quarters = [{'name': f'Quarter {num + 1}', 'f': s_from, 't': s_from + timedelta(days=s_day)}, {'name': f'Quarter {num + 2}', 'f': s_from + timedelta(days=s_day + 1), 't': s_from + timedelta(days=(s_to - s_from).days - 1)}, {'name': f'Final Exam {final}', 'f': s_to, 't': s_to}]
            for q in quarters:
                self.env['op.academic.term'].create({'name': q['name'], 'term_start_date': q['f'], 'term_end_date': q['t'], 'academic_year_id': self.id, 'parent_term': sub_term.id})
            num += 2
            final += 1
    ...
    def _handle_three_sem(self):
        ...
        res = [{'name': 'Semester 1', 'f': from_d, 't': from_d + timedelta(days=day)}, {'name': 'Semester 2', 'f': from_d + timedelta(days=day + 1), 't': from_d + timedelta(days=day * 2 + 1)}, {'name': 'Semester 3', 'f': from_d + timedelta(days=day * 2 + 2), 't': to_d}]
        for t in res:
            self.env['op.academic.term'].create({'name': t['name'], 'term_start_date': t['f'], 'term_end_date': t['t'], 'academic_year_id': self.id})
