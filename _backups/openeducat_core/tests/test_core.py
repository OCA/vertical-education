...
class TestCore(TestCoreCommon):
    ...
    def test_case_3_faculty(self):
        ...
        new_faculty = self.op_faculty.create({'partner_id': self.env.ref('openeducat_core.op_res_partner_30').id, 'first_name': 'binapshah', 'last_name': 'kalol', 'birth_date': '2009-01-01', 'gender': 'male'})
    ...
    def test_case_6_student(self):
        ...
        vals = {'first_name': 'nikul', 'middle_name': 'm', 'last_name': 'ahir', 'birth_date': '1999-01-01', 'gender': 'm', 'partner_id': self.env.ref('openeducat_core.op_res_partner_14').id}
    ...
    def test_case_7_subject_registartion(self):
        ...
        self.env['op.student.course'].create({'student_id': student.id, 'course_id': course.id, 'batch_id': batch.id})
        vals = {'student_id': student.id, 'course_id': course.id, 'batch_id': batch.id}
