...
class OpRoomDistribution(models.TransientModel):
    ...
    exam_session = fields.Many2one('op.exam.session')
    ...
    def default_get(self, fields):
        ...
        reg_ids = self.env['op.subject.registration'].search([('course_id', '=', session.course_id.id)])
        student_ids = []
        for reg in reg_ids:
        ...
        res.update({'exam_id': active_id, 'name': exam.name, 'start_time': exam.start_time, 'end_time': exam.end_time, 'exam_session': session.id, 'course_id': session.course_id.id, 'batch_id': session.batch_id.id, 'total_student': total_student, 'student_ids': [(6, 0, student_ids)]})
        return res
    ...
    def schedule_exam(self):
        ...
        existing_attendees_for_this_exam = attendance_model.search([('exam_id', '=', self.exam_id.id)])
        if existing_attendees_for_this_exam:
            existing_attendees_for_this_exam.unlink()
        booked_rooms = attendance_model.search([('room_id', 'in', self.room_ids.ids), ('exam_id.start_time', '<', self.end_time), ('exam_id.end_time', '>', self.start_time), ('exam_id.state', 'not in', non_conflict_exam_states), ('exam_id', '!=', self.exam_id.id)])
        if booked_rooms:
            booked_room_names = ', '.join(booked_rooms.mapped('room_id.name'))
            raise ValidationError(_('The selected rooms (%s) are already booked for the specified time by other active exams.') % booked_room_names)
        conflicting_students = attendance_model.search([('student_id', 'in', self.student_ids.ids), ('exam_id.start_time', '<', self.end_time), ('exam_id.end_time', '>', self.start_time), ('exam_id.state', 'not in', non_conflict_exam_states), ('exam_id', '!=', self.exam_id.id)])
        if conflicting_students:
            conflicting_student_names = ', '.join(conflicting_students.mapped('student_id.name'))
            raise ValidationError(_('Students (%s) are already scheduled for another active exam during the specified time.') % conflicting_student_names)
        for exam_wiz in self:
            if exam_wiz.total_student > exam_wiz.room_capacity:
                raise exceptions.AccessError(_('Room capacity must be greater than total number of student'))
            ...
            for room in exam_wiz.room_ids:
                ...
                while student_ids_to_assign and assigned_in_room < room_current_capacity:
                    ...
                    attendance_model.create({'exam_id': exam_wiz.exam_id.id, 'student_id': student_id, 'status': 'present', 'course_id': exam_wiz.course_id.id, 'batch_id': exam_wiz.batch_id.id, 'room_id': room.id})
                    assigned_in_room += 1
