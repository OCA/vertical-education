...
class OpExam(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('schedule', 'Scheduled'), ('held', 'Held'), ('result_updated', 'Result Updated'), ('cancel', 'Cancelled'), ('done', 'Done')], readonly=True, default='draft', tracking=True)
    note = fields.Text()
    ...
    _sql_constraints = [('unique_exam_code', 'unique(exam_code)', 'Code should be unique per exam!')]
    ...
    def _check_date_time(self):
        for record in self:
            ...
            if start_time_dt and end_time_dt:
                ...
                elif start_time_dt < session_start_dt or start_time_dt > session_end_dt or end_time_dt < session_start_dt or (end_time_dt > session_end_dt):
                    raise ValidationError(_('Exam Time should be within the Exam Session Dates.'))

    def open_exam_attendees(self):
        self.ensure_one()
        return {'type': 'ir.actions.act_window', 'res_model': 'op.exam.attendees', 'domain': [('exam_id', '=', self.id)], 'name': 'Students', 'view_mode': 'list,form', 'context': {'default_exam_id': self.id}, 'target': 'current'}
    ...
    def _compute_results_entered(self):
        for record in self:
            record.results_entered = any((attendee.marks is not False and attendee.marks is not None for attendee in record.attendees_line))
    ...
    def _check_overlapping_times(self):
        for record in self:
            if not record.subject_id or not record.start_time or (not record.end_time):
                continue
            existing_exams = self.env['op.exam'].search([('subject_id', '=', record.subject_id.id), ('id', '!=', record.id), ('start_time', '<', record.end_time), ('end_time', '>', record.start_time)])
            if existing_exams:
                raise ValidationError(_(f"The exam time overlaps with an existing exam for the same subject : {', '.join(existing_exams.mapped('name'))}"))
