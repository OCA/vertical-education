...
class OpExam(models.Model):
        ...
        for record in self:
            ...
            if record.min_marks > record.total_marks:
                raise ValidationError(
                    _("Passing Marks can't be greater than Total Marks")
                )
        ...
        for record in self:
            ...
            if start_time_dt and end_time_dt:
                if start_time_dt > end_time_dt:
                    ...
                    start_time_dt < session_start_dt
                    or start_time_dt > session_end_dt
                    or end_time_dt < session_start_dt
                    or end_time_dt > session_end_dt
                ):
                    raise ValidationError(
    ...
    def act_cancel(self):
        ...
        for exam in self:
            ...
            attendees_for_exam = self.env["op.exam.attendees"].search(
                [("exam_id", "=", exam.id)]
            )
