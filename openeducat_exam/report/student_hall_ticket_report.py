...
class ReportTicket(models.AbstractModel):
    ...
    def get_subject(self, exam_session):
        ...
        for exam_line in exam_session['exam_ids']:
            ...
            overlap = any((schedule_start < existing_end and schedule_end > existing_start for (existing_start, existing_end) in time_slots))
            if not overlap:
                res1 = {'subject': exam_line.subject_id.name, 'date': fields.Datetime.to_string(exam_line.start_time)[:10], 'time': self.get_date(exam_line), 'sup_sign': ''}
                lst.append(res1)
    ...
    def get_data(self, data):
        ...
        if exam_session:
            student_search = self.env['op.student'].search([('course_detail_ids.course_id', '=', exam_session.course_id.id)])
            for student in student_search:
                student_course = self.env['op.student.course'].search([('student_id', '=', student.id), ('course_id', '=', exam_session.course_id.id)])
                res = {'exam': exam_session.name, 'exam_code': exam_session.exam_code, 'course': exam_session.course_id.name, 'student': student.name, 'image': student.image_1920, 'roll_number': student_course.roll_number, 'line': self.get_subject(exam_session)}
    ...
    def _get_report_values(self, docids, data=None):
        ...
        docargs = {'doc_ids': self.ids, 'doc_model': model, 'docs': docs, 'time': time, 'get_data': self.get_data(data)}
        return docargs